...
import json
from config import COL_SEP, SAMPLE_SEP, MAX_INPUT_LENGTH, MAX_OUTPUT_LENGTH
...
class Processor:
    def __init__(self,
        ...
        self.allowed_modes = ["structured_output", "human_readable_output"]
        assert self.mode in self.allowed_modes

    def get_input_prompt(self, example):
        question, headers = example["question"], example["table"]["header"]
